...
/**
 * Template Engine Class for HTML Generation
 */
class TemplateEngine {
    ...
    public function __construct($template_dir = null) {
        $this->template_dir = $template_dir ?: plugin_dir_path(__FILE__) . 'templates/';
    }
    ...
    public function render($template_name, $variables = []) {
        ...
        $template_path = $this->template_dir . $template_name . '.html';
     ...
     */
    public function renderInline($template_content, $variables = []) {
        $this->setMultiple($variables);
...
/**
 * Content Generator Class for HTML Generation
 */
class ContentGenerator {
    ...
    /**
     * Generate package image HTML
     */
    public function generatePackageImage($image_url, $affiliate_url, $title, $width = '', $height = '') {
        if (empty($image_url)) return '';
        
        return $this->template_engine->render('package-image', [
            'package-image' => $image_url,
            'aff-link' => $affiliate_url,
            'title' => $title,
            'package-width' => $width,
     ...
     */
    public function generateSampleImages($images, $title, $max_images = 10, $show_caption = true, $show_link = true, $size = 'normal') {
        ...
        if ($size === 'small') {
            // Small size images in flex layout
            $html = "<!-- wp:html -->\n";
            $html .= "<div style=\"display: flex; flex-wrap:wrap; justify-content: start;\">\n";
            ...
            foreach ($images as $image) {
                ...
                if ($show_link) {
                    $html .= "<div style=\"padding: 5px; width: 110px;\"><a href=\"{$image}\" target=\"_blank\" rel=\"noreferrer noopener\">";
                    $html .= "<img src=\"{$image}\" alt=\"{$title} 画像" . ($count + 1) . "\" width=\"{$width}\" height=\"{$height}\"/></a></div>\n";
                } else {
                    $html .= "<div style=\"padding: 5px; width: 110px;\">";
                    $html .= "<img src=\"{$image}\" alt=\"{$title} 画像" . ($count + 1) . "\" width=\"{$width}\" height=\"{$height}\"/></div>\n";
                }
            ...
            foreach ($images as $image) {
                ...
                $html .= $this->template_engine->render('sample-images', [
                    'image-url' => $image,
                    'image-alt' => $title . ' 画像' . ($count + 1),
                    'image-width' => $width,
                    'image-height' => $height
                ]);
    ...
    /**
     * Generate detail table HTML
     */
    public function generateDetailTable($details) {
        if (empty($details)) return '';
        
        $rows = '';
        foreach ($details as $detail) {
            $rows .= "<tr><th>{$detail['label']}</th><td>{$detail['value']}</td></tr>";
        }
        
        return $this->template_engine->render('detail-table', [
            'detail-rows' => $rows
        ]);
    }
    
    /**
     * Generate sample movie HTML
     */
    public function generateSampleMovie($cid, $aff_id, $movie_size = '720_480') {
        $size_map = [
            '476' => ['width' => '476', 'height' => '306'],
            '560' => ['width' => '560', 'height' => '360'],
            '644' => ['width' => '644', 'height' => '414'],
            '600' => ['width' => '600', 'height' => '500'],
            '720' => ['width' => '720', 'height' => '480']
        ];
        
        $size = $size_map[$movie_size] ?? ['width' => '100%', 'height' => '100%'];
        
        return $this->template_engine->render('sample-movie', [
            'movie-width' => $size['width'],
            'movie-height' => $size['height'],
            'aff-id' => $aff_id,
            'cid' => $cid,
     ...
     */
    public function generateButton($button_number, $affiliate_url, $button_text, $text_color, $bg_color) {
        return $this->template_engine->render('button', [
            'button-number' => $button_number,
            'aff-link' => $affiliate_url,
            'button-text' => $button_text,
            'button-text-color' => $text_color,
            'button-bg-color' => $bg_color
        ]);
    }
    
    /**
     * Generate API mark HTML
     */
    public function generateApiMark($site = 'FANZA') {
        $api_image_url = ($site === 'FANZA') 
            ? 'https://p.dmm.co.jp/p/affiliate/web_service/r18_135_17.gif'
            : 'https://pics.dmm.com/af/web_service/com_135_17.gif';
        
        $api_alt_text = ($site === 'FANZA') 
            ? 'WEB SERVICE BY FANZA'
            : 'WEB SERVICE BY DMM.com';
        
        return $this->template_engine->render('api-mark', [
            'api-image-url' => $api_image_url,
            'api-alt-text' => $api_alt_text
        ]);
    }
     ...
     */
    public function generatePostContent($data, $template_content = '') {
        $variables = [
            'title' => $data['title'] ?? '',
            'cid' => $data['cid'] ?? '',
            'aff-link' => $data['affiliate_url'] ?? '',
            'package-image' => $data['package_image'] ?? '',
            'package-width' => $data['package_width'] ?? '',
            'package-height' => $data['package_height'] ?? '',
            'button-text' => $data['button_text'] ?? '詳細を見る',
            'button-text-color' => $data['button_text_color'] ?? '#ffffff',
            'button-bg-color' => $data['button_bg_color'] ?? '#007cba'
        ];
        
        if (!empty($template_content)) {
            return $this->template_engine->renderInline($template_content, $variables);
        }
...
class FANZAAutoPlugin {
    ...
    function post_set_page($set_menu_number){
        ...
        <div class="fanza-auto">
            <h2 class="mb-4">FANZA Auto大量投稿プラグイン | 投稿設定<?= $set_menu_number ?></h2>

            <form action="<?php echo admin_url('admin.php'); ?>" method="POST" id="dmm-add-form">
        ...
        <div class="collapse-header" id="collapseSetting" data-toggle="collapse" href="#collapseExample" role="button" aria-expanded="false" aria-controls="collapseExample">
            <div class="d-flex justify-content-between align-items-center">
                <span><strong>投稿設定<?= $set_menu_number ?>：初期設定</strong></span>
                <span id="collapse_triangle">▼</span>
            </div>
        </div>
    ...
    function post_contents_process($list, $post_content, $wpdb, $cont_title, $cont_comment, $set_menu_number="1"){
        ...
        if (isset($list->imageURL->large)) {
            ...
            $package_img = $content_generator->generatePackageImage($image_large, $affiliate, $title, $width, $height);
        }

        // Detail data for template system
        $detail_data = [];
        
        if ($review_html2 != '') {
            $detail_data[] = ['label' => 'レビュー', 'value' => $review_html2];
        }
        if (isset($onDate)) {
            $detail_data[] = ['label' => '発売日', 'value' => $onDate];
        }
        if (!empty($volume)) {
            if($service === 'ebook') {
                $volume .= 'ページ';
            ...
            $detail_data[] = ['label' => '収録', 'value' => $volume];
        }
        // Series
        if (!empty($series)) {
            $series_array = array_column($series, 'name');
            $series_str =  implode(" ", $series_array);
            $detail_data[] = ['label' => 'シリーズ', 'value' => $series_str];
        }
        // Author
        if (!empty($author)) {
            $author_array = array_column($author, 'name');
            $author_str =  implode(" ", $author_array);
            $detail_data[] = ['label' => '作者', 'value' => $author_str];
        }
        // Genres 
        if (!empty($genres)) {
            $genres_array = array_column($genres, 'name');
            $genre_str =  implode(" ", $genres_array);
            $detail_data[] = ['label' => 'ジャンル', 'value' => $genre_str];
        }
        // Actor
        if (!empty($actor)) {
            if ($floor == 'cd') {
            ...
            $actor_str = implode(" ", $actor_array);
            $detail_data[] = ['label' => ($floor == 'cd' ? 'アーティスト' : '出演者'), 'value' => $actor_str];
        }
        // Actress
        if (!empty($actress)) {
            $actress_array = array_column($actress, 'name');
            $actress_str =  implode(" ", $actress_array);
            $detail_data[] = ['label' => '女優', 'value' => $actress_str];
        }
        // Performer
        if (!empty($performer)) {
            $detail_data[] = ['label' => '出演者', 'value' => $performer];
        }
        // Director
        if (!empty($director)) {
            $director_array = array_column($director, 'name');
            $director_str =  implode(" ", $director_array);
            $detail_data[] = ['label' => '監督', 'value' => $director_str];
        }
        // Maker
        if (!empty($maker)) {
            ...
            $detail_data[] = ['label' => $maker_title_name, 'value' => $maker_str];
        }
        // Label
        if (!empty($label)) {
            $label_array = array_column($label, 'name');
            $label_str =  implode(" ", $label_array);
            $detail_data[] = ['label' => 'レーベル', 'value' => $label_str];
        }
        if (!empty($manufacture)) {
            $manufacture_array = array_column($manufacture, 'name');
            $manufacture_str =  implode(" ", $manufacture_array);
            $detail_data[] = ['label' => '出版社', 'value' => $manufacture_str];
        }

        // CID
        if (isset($cid)) {
            $detail_data[] = ['label' => '品番', 'value' => $cid];
        }
        // Jan Code
        if (isset($jancode)) {
            $detail_data[] = ['label' => 'JANコード', 'value' => $jancode];
        }
        // Maker Product
        if (isset($maker_product)) {
            $detail_data[] = ['label' => 'メーカー品番', 'value' => $maker_product];
        }
        // ISBN
        if (isset($isbn)) {
            $detail_data[] = ['label' => 'ISBN', 'value' => $isbn];
        }
        // Price
        if (isset($price)) {
            $detail_data[] = ['label' => '価格', 'value' => '￥' . $price];
        }
        
        // Generate detail table using template system
        $detail_table_template = $content_generator->generateDetailTable($detail_data);
        ...
        // Button
        $button_content = $content_generator->generateButton(1, $affiliate, $b_word, $t_color, $b_color);

        // Button2
        $button_content2 = $content_generator->generateButton(2, $affiliate, $b_word2, $t_color2, $b_color2);
        ...
        if(isset($list->sampleImageURL) || ($scraping_large_img_arry)) {
            ...
            if (!empty($large_image_arry)) {
                // Generate sample images using template system
                $sample_img_content = $content_generator->generateSampleImages($large_image_arry, $title, $maximage, true, true);
                $sample_img_content_no = $content_generator->generateSampleImages($large_image_arry, $title, $maximage, false, true);
                $sample_img_non = $content_generator->generateSampleImages($large_image_arry, $title, $maximage, true, false);
                $sample_no_cap_link = $content_generator->generateSampleImages($large_image_arry, $title, $maximage, false, false);
                            
            } else {
                // Process small images for digital_pcgame
                if ( $floor === 'digital_pcgame') {
                    $processed_images = array_map(function($img) {
                        return str_replace('js', 'jp', $img);
                    }, $small_image_arry);
                } else {
                    $processed_images = $small_image_arry;
                }
                
                // Generate sample images using template system
                $sample_img_content = $content_generator->generateSampleImages($processed_images, $title, $maximage, true, true);
                $sample_img_content_no = $content_generator->generateSampleImages($processed_images, $title, $maximage, false, true);
                $sample_img_non = $content_generator->generateSampleImages($processed_images, $title, $maximage, true, false);
                $sample_no_cap_link = $content_generator->generateSampleImages($processed_images, $title, $maximage, false, false);

                // Generate small sample images using template system
                $sample_img_content_small = $content_generator->generateSampleImages($processed_images, $title, $maximage, false, false, 'small');
            }
            if (!empty($large_image_arry) && !empty($small_image_arry)) {
                // Generate small sample images with links using template system
                $sample_img_content_small = $content_generator->generateSampleImages($large_image_arry, $title, $maximage, false, true, 'small');
            }
        }

        // API Mark
        $api_content = $content_generator->generateApiMark($site);
        ...
        $smovie_content = '';
        if (isset($list->sampleMovieURL)) {
            $smovie_content = $content_generator->generateSampleMovie($cid, $aff_id, $movie_size);
        }
    ...
    function make_post($post, $category, $tags, $post_thumbnail) {
        ...
        if ($id) {
            ...
            if (!empty($category) && $category[0] != "") {
                ...
                if ($default_category) {
                    $category = array($default_category);
