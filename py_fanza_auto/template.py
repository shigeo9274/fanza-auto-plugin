...
class Renderer:
    ...
    def generate_package_image(self, item: Dict[str, Any], affiliate_url: str, title: str = '', width: str = '', height: str = '') -> str:
        """パッケージ画像HTMLを生成（PHPのContentGenerator::generatePackageImage相当）"""
        ...
        return f'''<div class="package-image-container">
...
<tbody>'''
    ...
    def generate_sample_movie(self, cid: str, aff_id: str, movie_size: str = '720_480') -> str:
        """サンプル動画HTMLを生成（PHPのContentGenerator::generateSampleMovie相当）"""
        if not cid or not aff_id:
            return ''
        
        # 動画サイズの設定
        if movie_size == 'auto':
            width, height = '640', '360'
        elif movie_size == '720_480':
            width, height = '720', '480'
        elif movie_size == '1280_720':
            width, height = '1280', '720'
        else:
            width, height = '640', '360'
        
        return f'''<div class="sample-movie-container" style="text-align:center; margin: 20px 0;">
<iframe src="https://www.dmm.co.jp/litevideo/-/part/=/cid={cid}/aff_id={aff_id}/" width="{width}" height="{height}" frameborder="0" style="max-width: 100%; height: auto;"></iframe>
</div>'''
...
<!-- /wp:group -->'''
    ...
    def _get_short_comment(self, comment: str, max_length: int = 100) -> str:
        """コメントを短縮する"""
        if not comment:
            return ''
    ...
    def render_template(self, template_content: str, item: Dict[str, Any], 
        ...
        content = content.replace('[description]', item.get('description', ''))
        content = content.replace('[affiliate_url]', affiliate_url)
